Facts = new Mongo.Collection('biolog_facts');
...
Facts.attachSchema(
    new SimpleSchema({
        ...
        updater: { type: String, index: 1, optional: true, optional: true },
        deleter: { type: String, index: 1, optional: true, optional: true },
       ...
       //updated:Rashid - 11-Sept 2015  
        postId: { type: String, index:1, optional: true },
        likes: { type: [String], index:1, optional: true }
  })
);
...
if (Meteor.isServer) {
  Facts.allow({
    ...
    update : function ( userId, options) {
        console.log( options)
