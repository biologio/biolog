...
getPatientConditions = function(patientId) {
    if (! patientId) return;
    return Facts.find({pred: Conditions.PREDICATE._id, subj: patientId, valid: 1 });
};

getPatientConditionsCurrent = function(patientId) {
    if (! patientId) return;
    var result = Facts.find({pred: Conditions.PREDICATE._id, subj: patientId, valid: 1, endFlag: 1 }, {sort: {startDate: -1}});
    return result;
};

getPatientConditionsPast = function(patientId) {
    if (! patientId) return;
    var result = Facts.find({pred: Conditions.PREDICATE._id, subj: patientId, valid: 1, endFlag: 0 }, {sort: {endDate: -1}});
    return result;
};

getPatientMeds = function(patientId) {
    if (! patientId) return;
    var result = Facts.find({pred: Medications.PREDICATE._id, subj: patientId, valid: 1 });
    return result;
};

getPatientMedsCurrent = function(patientId) {
    if (! patientId) return;
    var result = Facts.find({pred: Medications.PREDICATE._id, subj: patientId, valid: 1, endFlag: 1 }, {sort: {startDate: -1}});
    return result;
};
