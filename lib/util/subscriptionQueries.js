...
getPatientConditions = function(patientId) {
    if (! patientId) return;
    return biolog_facts.find({pred: conditionPredicate._id, subj: patientId, valid: 1 });
};
...
getPatientConditionsCurrent = function(patientId) {
    ...
    var result = biolog_facts.find({pred: conditionPredicate._id, subj: patientId, valid: 1, endFlag: 1 }, {sort: {startDate: -1}});
...
getPatientConditionsPast = function(patientId) {
    ...
    var result = biolog_facts.find({pred: conditionPredicate._id, subj: patientId, valid: 1, endFlag: 0 }, {sort: {endDate: -1}});
...
getPatientFlags = function(patientId) {
    if (! patientId) return;
    return biolog_facts.find({pred: flagPredicate._id, subj: patientId, valid: 1 });
};

getPatientMeds = function(patientId) {
    if (! patientId) return;

    var result = biolog_facts.find({pred: medicationPredicate._id, subj: patientId, valid: 1 });
    //console.log("getPatientMeds queries: " + JSON.stringify({pred: medicationPredicate._id, subj: patientId, valid: 1 }) + "\nretrieves: " + JSON.stringify(result));
    return result;
};
...
getPatientMedsCurrent = function(patientId) {
    ...
    var result = biolog_facts.find({pred: medicationPredicate._id, subj: patientId, valid: 1, endFlag: 1 }, {sort: {startDate: -1}});
    //console.log("getPatientMeds queries: " + JSON.stringify({pred: medicationPredicate._id, subj: patientId, valid: 1 }) + "\nretrieves: " + JSON.stringify(result));
    return result;
};
...
getPatientMedsPast = function(patientId) {
    ...
    var result = biolog_facts.find({pred: medicationPredicate._id, subj: patientId, valid: 1, endFlag: 0 }, {sort: {endDate: -1}});
    //console.log("getPatientMeds queries: " + JSON.stringify({pred: medicationPredicate._id, subj: patientId, valid: 1 }) + "\nretrieves: " + JSON.stringify(result));
    return result;
};
...
queryPatient = function(userid) {
    if (! userid) return;
    return biolog_entities.find({etypes: "patient", _id: userid});
};

//TODO support users being permitted to handle other patients
getUserPatients = function(userid) {
    if (! userid) return;
    return biolog_entities.find({etypes: "patient", owners: userid, valid: 1 });
};