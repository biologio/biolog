...
getPatientDob = function(pt) {
    ...
    if (!pt) return;
    var dobObj = biolog.BiologUtil.getValuePath(pt, "data.id/dob");
    console.log("getPatientDob:", dobObj);
    if (!dobObj) return;
...
setPatientDob = function(pt, dob) {
    ...
    biolog.BiologUtil.setValuePath(pt, "data.id/dob", fact);
};

getPatientSex = function(pt) {
    if (!pt) pt = getPatient();
    if (!pt) return;
    var obj = biolog.BiologUtil.getValuePath(pt, "data.id/sex");
    if (!obj) return;
    return obj.text;
};
...
setPatientSex = function(pt, sex) {
    ...
    biolog.BiologUtil.setValuePath(pt, "data.id/sex", fact);
};

getPatientNickname = function(pt) {
    if (!pt) pt = getPatient();
    if (!pt) return;
    var obj = biolog.BiologUtil.getValuePath(pt, "data.id/nickname");
    if (!obj) return;
    return obj.text;
};
...
setPatientNickname = function(pt, nickname) {
    ...
    biolog.BiologUtil.setValuePath(pt, "data.id/nickname", fact);
};
...
getPatientCountry = function(pt) {
    if (!pt) pt = getPatient();
    if (!pt) return;
    var obj = biolog.BiologUtil.getValuePath(pt, "data.geo/country");
    if (!obj) return;
    return obj.text;
};
...
setPatientCountry = function(pt, val) {
    ...
    biolog.BiologUtil.setValuePath(pt, "data.geo/country", fact);
};

getPatientZip = function(pt) {
    if (!pt) pt = getPatient();
    if (!pt) return;
    var obj = biolog.BiologUtil.getValuePath(pt, "data.geo/zip");
    if (!obj) return;
    return obj.text;
};
...
setPatientZip = function(pt, val) {
    ...
    biolog.BiologUtil.setValuePath(pt, "data.geo/zip", fact);
};


savePatientDemographics = function(pt) {
    if (!pt) pt = getPatient();
